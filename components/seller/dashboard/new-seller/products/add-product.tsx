...
function FeaturesEditor({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    // Markdown shortcuts on Space: "- ", "* " → bullet list, "1. " → numbered list
    if (e.key === " ") {
      const sel = window.getSelection()
      if (!sel || sel.rangeCount === 0) return
      ...
      // Only trigger if this is the ONLY text on the line (at the very start)
      const parentEl = textNode.parentElement
      const isTopLevel = parentEl === editorRef.current ||
        parentEl?.parentElement === editorRef.current
      ...
      if (textBeforeCaret === "-" || textBeforeCaret === "*") {
        ...
        deleteRange.setEnd(textNode, textBeforeCaret.length)
        deleteRange.deleteContents()
        sel.removeAllRanges()
        sel.addRange(deleteRange)
        document.execCommand("insertUnorderedList", false)
        setActiveBlock("Bulleted list")
        emitChange()
        return
      }
...
export function AddProduct({ categories, onSave, onCancel, editingProduct, products = [], onEditProduct, onDeleteDraft }: AddProductProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Upload ALL in parallel — no more waiting one by one
  ...
  const handlePublish = () => {
    ...
    // Block if images are still uploading (blob URLs not yet replaced by imgbb URLs)
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      {/* Edit Published Tab — fetches full product before opening edit form */}
      {activeSubTab === "edit" && (
        <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            {searchedPublished.length === 0 ? (
              ...
              searchedPublished.map((product: any) => (
                <div key={product.id}
                  onClick={async () => {
                    try {
                      // Fetch full product with all details before opening edit form
                      const token = localStorage.getItem("seller_token")
                      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products/${product.id}`, {
                        headers: { Authorization: `Bearer ${token}` }
                      })
                      const json = await res.json()
                      // mapDbToProduct converts DB field names to form field names
                      const { mapDbToProduct: mapFn } = await import("@/lib/supabase")
                      const fullProduct = json.product ? mapFn(json.product) : product
                      if (onEditProduct) onEditProduct(fullProduct)
                    } catch {
