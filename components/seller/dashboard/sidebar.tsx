...
const getManagementItems = (isUsed: boolean) => [
...
export function SellerSidebar({ collapsed = false, onToggle }: SidebarProps) {
  ...
  const { pendingCount, setPendingCount } = useOrders()
  const menuItems = getMenuItems(pendingCount, isUsed)
  const managementItems = getManagementItems(isUsed)
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("seller_pending_count")
  ...
  useEffect(() => {
    ...
    // Get seller_id for Realtime subscription
