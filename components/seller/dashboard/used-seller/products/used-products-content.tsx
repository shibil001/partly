...
import {
  ...
  Trash2, Eye, ChevronLeft, ChevronRight, RefreshCw,
  AlertTriangle, MapPin, Car, Calendar, Tag, Filter
} from "lucide-react"
import { AddProduct } from "@/components/seller/dashboard/used-seller/products/add-product"
...
const conditionConfig: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  "New":             { bg: "bg-green-50",  text: "text-green-700",  border: "border-green-200", dot: "bg-green-500" },
  "Used - Like New": { bg: "bg-blue-50",   text: "text-blue-700",   border: "border-blue-200",  dot: "bg-blue-500" },
  "Used - Good":     { bg: "bg-yellow-50", text: "text-yellow-700", border: "border-yellow-200",dot: "bg-yellow-500" },
  "Used - Fair":     { bg: "bg-orange-50", text: "text-orange-700", border: "border-orange-200",dot: "bg-orange-500" },
  "Refurbished":     { bg: "bg-purple-50", text: "text-purple-700", border: "border-purple-200",dot: "bg-purple-500" },
}
...
export function UsedProductsContent() {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const matchesSearch = !q ||
      ...
      (p as any).partNumber?.toLowerCase().includes(q) ||
      p.brand?.toLowerCase().includes(q) ||
      (p as any).vehicleModel?.toLowerCase().includes(q)
  ...
  return (
    <div className="flex-1 p-6 overflow-auto bg-gray-50">
      ...
      <div className="flex gap-1 mb-6 bg-white border border-gray-200 rounded-xl p-1 w-fit">
        {[
          { id: "products", label: "Parts List", icon: <Package className="h-4 w-4" /> },
      ...
      {activeTab === "products" && (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Used Parts</h2>
              <p className="text-sm text-gray-500">
                {loading ? "Loading..." : `${filteredProducts.length} parts listed`}
              </p>
            </div>
            <button
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search name, part no, vehicle..."
                  value={search}
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-1">
                <button onClick={() => setViewMode("card")}
                  className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${viewMode === "card" ? "bg-blue-600 text-white" : "text-gray-500 hover:text-gray-700"}`}>
                  Cards
                </button>
                <button onClick={() => setViewMode("table")}
                  className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${viewMode === "table" ? "bg-blue-600 text-white" : "text-gray-500 hover:text-gray-700"}`}>
                  Table
                </button>
              </div>
              <button
          ...
          {loading ? (
            ...
            <div className="py-16 text-center bg-white rounded-xl border border-gray-200">
              ...
              <p className="text-sm font-medium text-gray-500">No used parts found</p>
              <p className="text-xs text-gray-400 mt-1 mb-4">Add your first used part to get started</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {paginatedProducts.map((product) => {
                ...
                const condStyle = conditionConfig[cond] || { bg: "bg-gray-50", text: "text-gray-600", border: "border-gray-200", dot: "bg-gray-400" }
                const statStyle = statusConfig[product.status] || { bg: "bg-gray-50", text: "text-gray-600", border: "border-gray-200" }
                return (
                  <div key={product.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md hover:border-blue-200 transition-all">
                    <div className="flex gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full border font-medium shrink-0 ${statStyle.bg} ${statStyle.text} ${statStyle.border}`}>
                            {product.status}
                          </span>
                        </div>
                        {product.brand && <p className="text-[11px] text-gray-400 mt-0.5">{product.brand}{(product as any).partNumber ? ` · #${(product as any).partNumber}` : ""}</p>}

                        {/* Condition badge */}
                        {cond && (
                          <span className={`inline-flex items-center gap-1 mt-1.5 text-[10px] px-2 py-0.5 rounded-full border font-medium ${condStyle.bg} ${condStyle.text} ${condStyle.border}`}>
                    ...
                    <div className="px-4 pb-3 grid grid-cols-3 gap-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] text-gray-600 truncate">{(product as any).vehicleModel || <span className="text-gray-300">—</span>}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Calendar className="h-3 w-3 text-gray-400 shrink-0" />
                        <span className="text-[11px] text-gray-600">{(product as any).year || <span className="text-gray-300">—</span>}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <MapPin className="h-3 w-3 text-gray-400 shrink-0" />
                        <span className="text-[11px] text-gray-600 truncate">{product.district || product.state || <span className="text-gray-300">—</span>}</span>
                      </div>
                    </div>
                    ...
                    <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <button onClick={() => window.open(`/product/${product.id}`, "_blank")}
                          className="p-1.5 text-gray-400 hover:text-blue-600 transition-colors rounded-lg hover:bg-blue-50" title="View listing">
                          <Eye className="h-4 w-4" />
                        </button>
                        <button onClick={() => handleEdit(product)}
                          className="p-1.5 text-gray-400 hover:text-blue-600 transition-colors rounded-lg hover:bg-blue-50" title="Edit">
                          <Pencil className="h-4 w-4" />
                        </button>
                        <button onClick={() => requestDelete(product.name, () => deleteProduct(product.id))}
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left text-xs font-medium text-gray-500 py-3 px-2">Qty</th>
                      ...
                      <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="py-3 px-4">
                          <input type="checkbox" checked={selectedProducts.includes(product.id)}
                            onChange={() => toggleSelect(product.id)} className="rounded border-gray-300" />
                        </td>
                        <td className="py-3 px-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-semibold text-gray-900 line-clamp-2 max-w-[160px]">{product.name}</p>
                              <p className="text-[10px] text-gray-400 mt-0.5">{product.brand}{(product as any).partNumber ? ` · ${(product as any).partNumber}` : ""}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-2"><span className="text-xs text-gray-700">{(product as any).vehicleModel || "—"}</span></td>
                        <td className="py-3 px-2"><span className="text-xs text-gray-700">{(product as any).year || "—"}</span></td>
                        <td className="py-3 px-2">
                          <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border font-medium ${condStyle.bg} ${condStyle.text} ${condStyle.border}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${condStyle.dot}`} />
                            {cond || "—"}
                          </span>
                        </td>
                        <td className="py-3 px-2"><span className="text-xs text-gray-700">{product.stock ?? 1}</span></td>
                        <td className="py-3 px-2"><span className="text-xs font-semibold text-gray-900">₹{product.price?.toLocaleString("en-IN")}</span></td>
                        <td className="py-3 px-2">
                          {product.district || product.state ? (
                            <div className="flex items-center gap-1 text-[10px] text-gray-500">
                              <MapPin className="h-3 w-3 shrink-0 text-gray-400" />
                              <span className="truncate max-w-[90px]">{[product.district, product.state].filter(Boolean).join(", ")}</span>
                            </div>
                        ...
                        <td className="py-3 px-2">
                          <div className="flex items-center gap-1">
                            <button onClick={() => window.open(`/product/${product.id}`, "_blank")}
                              className="p-1 text-gray-400 hover:text-blue-600 transition-colors" title="View">
                              <Eye className="h-4 w-4" />
                            </button>
                            <button onClick={() => handleEdit(product)}
            ...
            {categories.map((cat) => {
              ...
              return (
                <div key={cat.id} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-200 transition-colors">
                  ...
                  <p className="text-xs text-gray-400 mt-3">{cat.description}</p>
                </div>
              )
            })}
          </div>
        ...
        <AddProduct
          ...
          onEditProduct={async (product) => {
            try {
              ...
              const json = await res.json()
              setEditingProduct(json.product ? mapDbToProduct(json.product) : product)
            } catch {
              setEditingProduct(product)
            }
          }}
