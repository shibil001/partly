...
import {
  ...
  Upload, X, Plus, FileText, Search, Truck, Zap, Check, Settings,
        ...
        <div className="px-5 pb-5">
          <div className="border border-gray-200 rounded-xl overflow-visible">
            ...
            <div
              ...
              data-placeholder="Enter additional details — condition notes, history, known defects, modifications..."
...
export function AddProduct({ categories, onSave, onCancel, editingProduct, products = [], onEditProduct, onDeleteDraft }: AddProductProps) {
  ...
  const [form, setForm] = useState({ ...emptyForm, customFields: [] as { label: string; value: string }[] })
  ...
  useEffect(() => {
    if (editingProduct) {
      setForm({
        ...
        originalPrice: "",  // used seller has no original price field
        ...
        // District isn't stored separately — parse from location string "State, District, Pincode"
        district: editingProduct.district || (() => {
          const loc = editingProduct.location || ""
          const parts = loc.split(",").map((s: string) => s.trim())
          // Format is "State, District, Pincode" — district is middle part
        ...
        allowMakeOffer: editingProduct.allowMakeOffer !== false,
        allowMessage: editingProduct.allowMessage !== false,
  ...
  const handleSellingPriceChange = (value: string) => {
    ...
    const orig = parseFloat(form.originalPrice) || 0
    // Only validate if there's an actual original price set (new seller only)
    if (orig > 0 && form.originalPrice !== "") {
      validatePrice(selling, orig)
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Show ALL previews instantly before any upload starts
    ...
    await Promise.all(validFiles.map(async (file, i) => {
      ...
      try {
        ...
        if (imgData.success) {
          setForm(p => ({ ...p, images: p.images.map(u => u === localUrl ? imgData.data.url : u) }))
          URL.revokeObjectURL(localUrl)
        } else {
          setForm(p => ({ ...p, images: p.images.filter(u => u !== localUrl) }))
          URL.revokeObjectURL(localUrl)
          setImageError("One or more images failed to upload, try again")
        }
      } catch {
        setForm(p => ({ ...p, images: p.images.filter(u => u !== localUrl) }))
        URL.revokeObjectURL(localUrl)
  ...
  const handlePublish = () => {
    ...
    const orig = parseFloat(form.originalPrice) || 0
    // Only validate original price if it was explicitly set (new seller has this field)
    if (orig > 0 && form.originalPrice !== "" && selling > orig) {
      setPriceError("Selling price can't be higher than original price")
      return
    }
    if (form.name.length > 180) { return }
    if (form.images.some(u => u.startsWith('blob:'))) {
      setImageError("Please wait — images are still uploading...")
      return
    }
  ...
  const buildProduct = () => ({
    ...
    originalPrice: parseFloat(form.price) || 0,  // used seller has no MRP field — set equal to price
    ...
    allowMakeOffer: form.allowMakeOffer,
    ...
    <div className="space-y-4 max-w-4xl">
      ...
      {activeSubTab === "new" && (
        <>
                      ...
                      <input type="file" accept="image/*" onChange={(e) => {
                        ...
                        ;(async () => {
                          try {
                            ...
                            const fd = new FormData(); fd.append("image", b64); fd.append("key", "02b4c7432d64053cdaebd47e3a9918ab")
                            const ir = await fetch("https://api.imgbb.com/1/upload", { method: "POST", body: fd })
                            const id = await ir.json()
                            if (id.success) setForm(p => ({ ...p, sizeChart: id.data.url }))
                          } catch {}
                        })()
                      }} className="hidden" />
                    </label>
          ...
          {/* Additional Details — WYSIWYG Editor */}
          <FeaturesEditor
            value={form.features}
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            >
              ...
              <ChevronUp className={`h-4 w-4 text-gray-400 transition-transform ${settingsOpen ? "" : "rotate-180"}`} />
            </button>
            {settingsOpen && (
              <div className="px-5 pb-5 border-t border-gray-100 space-y-5 pt-4">
                <div>
                  <p className="text-sm font-semibold text-gray-700 mb-3">Toggle On / Off</p>
                  <div className="space-y-3">
                    {/* Make an Offer toggle */}
                    <div className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl">
                      <div>
                        <p className="text-sm font-medium text-gray-800">Make an Offer</p>
                        <p className="text-xs text-gray-400 mt-0.5">Allow buyers to send you an offer via WhatsApp</p>
                      </div>
                      <div
                        onClick={() => setForm(p => ({ ...p, allowMakeOffer: !p.allowMakeOffer }))}
                        className={`w-11 h-6 rounded-full transition-colors flex items-center px-1 cursor-pointer ${form.allowMakeOffer ? "bg-blue-600" : "bg-gray-200"}`}
                      >
                        <div className={`w-4 h-4 rounded-full bg-white transition-transform shadow-sm ${form.allowMakeOffer ? "translate-x-5" : "translate-x-0"}`} />
                      </div>
                    </div>
                    {/* Message toggle */}
                    <div className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">Allow buyers to send you a message via WhatsApp</p>
                      </div>
                      <div
                        onClick={() => setForm(p => ({ ...p, allowMessage: !p.allowMessage }))}
                        className={`w-11 h-6 rounded-full transition-colors flex items-center px-1 cursor-pointer ${form.allowMessage ? "bg-blue-600" : "bg-gray-200"}`}
                      >
                        <div className={`w-4 h-4 rounded-full bg-white transition-transform shadow-sm ${form.allowMessage ? "translate-x-5" : "translate-x-0"}`} />
                      </div>
                    </div>
                  </div>
                </div>
